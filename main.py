...
class LSBAudioStego():
    ...
    def encodeAudio(self, audioLocation, stringToEncode):
        ...
        stringToEncode = stringToEncode + int((len(audioArray) - (len(stringToEncode) * 8 * 8)) / 8) * '#' #to concatenate string and int, type conversion intrger to string
        bits = list(map(int, ''.join([bin(ord(i)).lstrip('0b').rjust(8, '0') for i in stringToEncode]))) #convert string to list of bits
        # do lsb manipulation
